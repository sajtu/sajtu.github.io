...
# Debian based distro
	...
	#do system upgrade
	(/usr/bin/apt -y upgrade 2>&1 && aptRC=$? && echo "[EXIT_CODE] ${aptRC}") | while read stdout;
	do
	...
	# Add
	PACKAGES_LIST+=" systemd-timesyncd ufw libpam-pwquality net-tools iproute2 strace lsof gdb procps sysstat lshw dmidecode tree htop parted gparted gdisk ncdu iotop smartmontools traceroute dnsutils curl nmap telnet tcpdump whois bash-completion man-db less unzip rsync netcat-openbsd gnupg2 lsb-release curl software-properties-common";

	#do installtion of pre-requisits 
	(/usr/bin/apt -y install ${PACKAGES_LIST} 2>&1 && aptRC=$? && echo "[EXIT_CODE] ${aptRC}") | while read stdout;
	do
		chkforRC=`echo "${stdout}" 2>/dev/null | grep "^\[EXIT_CODE]" 2>/dev/null`;
		if [ -n "${chkforRC}" ]; then
			exitCode=`echo "${chkforRC}" 2>/dev/null | awk '{print \$2}' 2>/dev/null`;
